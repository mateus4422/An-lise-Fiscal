...
def efd():
    ...
    uploaded_files = st.file_uploader("Selecione os arquivos EFD (TXT e Latin-1):", type="txt",
    ...
    if uploaded_files is not None and len(uploaded_files) > 0:
        ...
        df_combined_all = pd.concat(df_combined_list, ignore_index=True)

        st.write(
            "Registros C100 seguidos dos registros C170 com identificação do registro na primeira coluna e colunas renomeadas:")
        st.write(df_combined_all)

        if st.button("Exportar CSV separado por '|'"):
            # Exporta o dataframe como um arquivo CSV separado por ';'
            csv = df_combined_all.to_csv(index=False, sep=";", encoding="utf-8-sig")
